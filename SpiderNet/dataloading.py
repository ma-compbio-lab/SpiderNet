...
## Load sample metadata and select Discovery–Untreated samples
metadata_sample = pd.read_csv(os.path.join(data_path_main, "HGSC_metadata.csv"), index_col=0)
metadata_sample_discovery = metadata_sample.loc[metadata_sample['dataset'] == 'Discovery', :]
metadata_sample_discovery_Untreated = metadata_sample_discovery.loc[
    metadata_sample_discovery['treatment'] == 'Untreated', :
]
...
for adata_files_cur in adata_files:
    ...
    ## Keep only Discovery–Untreated samples
    select_condition = np.isin(np.unique(adata_cur.obs['samples'])[0],
                               metadata_sample_discovery_Untreated.index)
    ...
    ## ------------------------------
    ## Merge cell class labels
    ## ------------------------------
    adata_cur.obs['cell.types'] = (
        adata_cur.obs['cell.types']
        .str.replace('B.cell_LC', 'B.cell')
        .str.replace('Endothelial_LC', 'Endothelial')
        .str.replace('Fibroblast_LC', 'Fibroblast')
        .str.replace('Malignant_LC', 'Malignant')
        .str.replace('Mast.cell_LC', 'Mast.cell')
        .str.replace('Monocyte_LC', 'Monocyte')
        .str.replace('TNK.cell_LC', 'TNK.cell')
    )

    ## ------------------------------
    ## Merge cell subtype labels
    ## ------------------------------
    adata_cur.obs['cell.subtypes'] = (
        adata_cur.obs['cell.subtypes']
        .str.replace('B.cell_LC', 'B.cell')
        .str.replace('CD4.T.cell.DN_LC', 'CD4.T.cell.DN')
        .str.replace('CD4.T.cell_LC', 'CD4.T.cell')
        .str.replace('CD8.T.cell_LC', 'CD8.T.cell')
        .str.replace('Endothelial_LC', 'Endothelial')
        .str.replace('Fibroblast_LC', 'Fibroblast')
        .str.replace('Malignant_LC', 'Malignant')
        .str.replace('Mast.cell_LC', 'Mast.cell')
        .str.replace('Monocyte_LC', 'Monocyte')
        .str.replace('NK.cell_LC', 'NK.cell')
        .str.replace('T.cell.DP_LC', 'T.cell.DP')
        .str.replace('Treg_LC', 'Treg')
        .str.replace('CD4.T.cell.DN', 'CD4.T.cell')
        .str.replace('T.cell.DP', 'T.B.cell')
    )
    del adata_cur
...
## ------------------------------
metadata_sample = pd.read_csv(os.path.join(data_path_main, "HGSC_metadata.csv"), index_col=0)
metadata_sample = metadata_sample.loc[np.sort(np.unique(batch_cell)), :]

metadata_sample["stage_index"] = metadata_sample["stage"].astype('category').cat.codes
metadata_sample["treatment_index"] = metadata_sample["treatment"].astype('category').cat.codes
metadata_sample["sites_binary_index"] = metadata_sample["sites_binary"].astype('category').cat.codes

metadata_sample['outcome_use'] = metadata_sample['outcome']
metadata_sample.loc[metadata_sample['outcome_use'] == 'Dead of disease',
                    'outcome_use'] = 'Dead (disease)'
metadata_sample["outcome_use_index"] = metadata_sample["outcome_use"].astype('category').cat.codes
metadata_sample['samples'] = metadata_sample.index
...
for sample in batch_cell_unique:
    ...
    data_dict = {
        ...
        'patients': adata_cur.obs['patients'],
        'cell_class': adata_cur.obs['cell.types'],
        'cell_subclass': adata_cur.obs['cell.subtypes'],
...
for idx in range(len(batch_cell_unique)):
    data_pyg = Data(
        ...
        cell_subclass=SpiderNet_data_list[idx]['cell_subclass'],
        ...
        patients=SpiderNet_data_list[idx]['patients']
    )
...
#################################################################
## Step 7: Compute and visualize mean LR co-expression per sample
#################################################################

meanLR_df = pd.DataFrame(index=batch_cell_unique,
                         columns=np.arange(len(LR_list)))

for sample in batch_cell_unique:
    sample_idx = np.where(batch_cell_unique == sample)[0][0]
    vals = SpiderNet_data_pyg_list[sample_idx]['cellpair_LRpair_neigh'].cpu().numpy()
    meanLR_df.loc[sample, :] = np.mean(vals, axis=0)

## Order by outcome
metadata_sample['outcome_use'] = metadata_sample['outcome_use'].fillna('Unknown')
metadata_order = metadata_sample.iloc[np.argsort(np.array(metadata_sample['outcome_use'])), :]
meanLR_df_order = meanLR_df.loc[metadata_order.index, :].astype(np.float32)

# Normalize columns (min-max)
meanLR_df_order = (meanLR_df_order - meanLR_df_order.min(axis=0)) / (
    meanLR_df_order.max(axis=0) - meanLR_df_order.min(axis=0)
)

# Plot clustered heatmap (columns only)
sns.clustermap(meanLR_df_order, cmap='vlag',
               col_cluster=True, row_cluster=False,
               linewidths=0.3, linecolor='black',
               figsize=(20, 30))
plt.savefig(save_path_main + "Mean_LR_pairs_heatmap_orderedbyoutcome.png",
            dpi=300, bbox_inches='tight')
plt.close()

## Order by treatment
metadata_sample['treatment'] = metadata_sample['treatment'].fillna('Unknown')
metadata_order = metadata_sample.iloc[np.argsort(np.array(metadata_sample['treatment'])), :]
meanLR_df_order = meanLR_df.loc[metadata_order.index, :].astype(np.float32)
meanLR_df_order = (meanLR_df_order - meanLR_df_order.min(axis=0)) / (
    meanLR_df_order.max(axis=0) - meanLR_df_order.min(axis=0)
)
...
sns.clustermap(meanLR_df_order, cmap='vlag',
               ...
               linewidths=0.3, linecolor='black',
               figsize=(20, 30))
plt.savefig(save_path_main + "Mean_LR_pairs_heatmap_orderedbytreatment.png",
            dpi=300, bbox_inches='tight')
plt.close()


#################################################################
## Step 8: Cell type composition across samples
#################################################################

# Crosstab of treatment × outcome
ct = pd.crosstab(metadata_sample['treatment'], metadata_sample['outcome_use'])
print(ct.to_string())

# Compute cell type proportions for each tissue slice
ctprop_df = pd.DataFrame(index=batch_cell_unique, columns=cellclass_unique)
for sample in batch_cell_unique:
    adata_cur = adata_list[np.where(batch_cell_unique == sample)[0][0]]
    cell_counts = np.unique(adata_cur.obs['cell.types'], return_counts=True)
    for ct in cellclass_unique:
        if ct not in cell_counts[0]:
            ctprop_df.loc[sample, ct] = 0
        else:
            ctprop_df.loc[sample, ct] = (
                cell_counts[1][np.where(cell_counts[0] == ct)[0]] / adata_cur.n_obs
            )

ctprop_df_order = ctprop_df.loc[metadata_order.index, :].astype(np.float32)
ctprop_df_order = ctprop_df_order / ctprop_df_order.max(axis=0)

sns.clustermap(ctprop_df_order, cmap='vlag',
               col_cluster=False, row_cluster=False,
               linewidths=0.3, linecolor='black',
               figsize=(10, 30))
plt.savefig(save_path_main + "ctprop_df_order.png",
            dpi=300, bbox_inches='tight')
plt.close()

# Example summary statistics for one cell type
celltype_show = 'TNK.cell'
alive_quantiles = np.quantile(
    np.array(ctprop_df_order[celltype_show])[
        np.where(np.array(metadata_order['outcome']) == "Alive")[0]
    ],
    (0, 0.2, 0.5, 0.8, 1.0)
)
print("Alive samples (TNK.cell) quantiles:", alive_quantiles)

top15_quantiles = np.quantile(
    np.array(ctprop_df_order[celltype_show])[::-1][:15],
    (0, 0.2, 0.5, 0.8, 1.0)
)
print("Top 15 samples quantiles:", top15_quantiles)

# Split proportions by site
ctprop_Adnexa = ctprop_df_order.iloc[
    np.where(metadata_order['sites_binary'] == 'Adnexa')[0], :
].astype(np.float32)

ctprop_Omentum = ctprop_df_order.iloc[
    np.where(metadata_order['sites_binary'] == 'Omentum')[0], :
].astype(np.float32)
...
cell_types = np.unique(adata_copy.obs['cell.types'])
...
# Save metadata
metadata_sample.to_csv(file_savepath_main + 'metadata_sample.csv', index=False)
