...
def Initial_model(SpiderNet_data_pyg_list,dim_envir,
                  ...
                  if_extend = True,threshold_crosscorr = 0.3,numtop_crosscorr = 10,spearcorr_use_rowmax_threshold = 0.3):
    ...
    if if_extend:
        ...
        if crosscorrelation_choose1[0].shape[0] > 0:
            ...
            exp_coexpression_extend_LR = np.empty(
                ...
                dtype=np.float32  # 或与你的数据类型一致
            )

            # 分块写入
            row_start = 0
            for i, m in enumerate(exp_coexpression_extend_list):
                sub = m[:, spearcorr_use_rowmax_index1].toarray()  # 先转dense
                n_rows = sub.shape[0]
    ...
    def fit(self, SpiderNet_data_pyg_list,
        ...
        for epoch in range(max_epoch):
            ...
            with autocast():
                ...
                ## Compute loss
                if epoch < warmup:
                    loss_factor_envir = self.compute_loss_in_batches(
            ...
            if epoch % int(max_epoch / 100) == 0 or epoch == max_epoch - 1:
                torch.save(self.state_dict(), file_savepath_model_main + 'model_epoch' + str(epoch) + '.pth')
