...
def Ligand_Receptor_gene_extraction_CellChatdb(ligand_receptor_filedir, adata):
    ...
    genenames_set = np.unique(np.array(genenames))  # Convert genenames to a set for faster lookup
    ...
    # Assume db_ligand_receptor is a DataFrame and genenames is a list
    #
    ...
    #
    # Use vectorized operations to filter valid ligand–receptor pairs
    ...
    ##
    print("The Ligand–receptor pair: " + str(len(LR_list_use)))
...
def Ligand_Receptor_gene_extraction_scSeqComm(ligand_receptor_filedir, adata):
    ...
    ## Remove rows that contain NaN values
    db_ligand_receptor = db_ligand_receptor.dropna(axis=0, how='any')
    ## Choose ligand–receptor pairs that exist in the genenames
    # Assume db_ligand_receptor is a DataFrame and genenames is a list
    genenames_set = set(genenames)  # Convert genenames to a set for faster lookup
    ...
    LR_choose_unique = list(set(tuple(map(tuple, pair)) for pair in LR_choose))
    ##
    print("The Ligand–receptor pair: " + str(len(LR_choose_unique)))
...
def scatter_nanmax(src: torch.Tensor, index: torch.Tensor, dim: int = 0, dim_size: int = None) -> torch.Tensor:
    """
    scatter_nanmax: Similar to scatter_max, but ignores NaN values and computes the maximum for each group.
    If all elements in a group are NaN, the result is set to NaN.
    """
    # mask: valid values = 1, NaN = 0
    mask = (~torch.isnan(src)).float()

    # Replace NaN with -inf so it doesn't affect the maximum
    src_no_nan = torch.nan_to_num(src, nan=float("-inf"))

    # scatter_max (returns values and indices)
    out, _ = scatter_max(src_no_nan, index, dim=dim, dim_size=dim_size)

    # Denominator: count of valid elements per group
    den = scatter_add(mask, index, dim=dim, dim_size=dim_size)

    # If all elements in a group are NaN (den = 0), manually set to NaN
    out[den == 0] = float("nan")
